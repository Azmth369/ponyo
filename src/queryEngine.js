...
function detectScope(q) {
  ...
  if (/\b(?:member|members|player|players|clan|donation|donations|donat\w*|donors?|trophies|town hall|role|elder|elders|leader|leaders|co-?leaders?)\b/.test(cleaned)) return 'clan';
  return 'general';
}
...
function buildFilters(q) {
  ...
  const unused =
    ...
    /\b(?:hasn'?t|haven'?t|didn'?t|didnt|has not|have not|did not|not)\s+(?:use|used|make|made|do|done|attack|attacked|hit|hits)\b/.test(q) ||
    /\b(?:yet to|without)\s+(?:use|make|do|attack|hit)(?:\s+(?:any\s+)?attacks?)?\b/.test(q) ||
    // Hinglish: "attack nahi kiya", "kisne attack nhi kia"
    /\battacks?\s+(?:nahi|nahin|nhi|nai)\b/.test(q);

  // "missed / skipped / pending / still needs to attack": anyone with at least
  // one attack left (superset of unused). Distinct from "unused" (zero used).
  const remainingAny =
    /\b(?:missed|miss|skipped|skip|forgot|forgotten|pending|incomplete|unfinished)\b/.test(q) ||
    /\b(?:still|yet)\s+(?:needs?|has|have|to)\s+(?:to\s+)?(?:attack|hit|use|make)/.test(q) ||
    /\b(?:remaining|left)\s+attacks?\b/.test(q) ||
    /\bwho(?:'s| is)?\s+(?:left|remaining)\b/.test(q);
  ...
  const asksLowest = /\b(?:lowest|least|minimum|min|bottom|worst)\b/.test(q);
  const asksHighest = /\b(?:highest|most|maximum|max|top|best)\b/.test(q);
  const donation = /\b(?:donat(?:ion|ions|ed|e|es|ing)?|donors?)\b/.test(q);
  ...
  // "top 5 donors" / "bottom 3": explicit list length.
  const limitMatch = q.match(/\b(?:top|bottom|first|last)\s+(\d{1,2})\b/);
  const limit = limitMatch ? Math.min(Number(limitMatch[1]), 50) : null;

  return {
    unused,
    attacks_remaining_any: remainingAny && !unused && remaining === null,
    limit,
    attacks_used: used,
    attacks_used_min: usedMin,
    attacks_remaining: remaining,
    sort: asksLowest && !/\btop\b/.test(q) ? 'asc' : asksHighest ? 'desc' : null,
...
export function buildQueryPlan(question = '') {
  ...
  if (asksClanIdentity) operation = 'clan_identity';
    ...
    if (filters.unused || filters.attacks_remaining_any || filters.attacks_used !== null || filters.attacks_used_min !== null || filters.attacks_remaining !== null) operation = 'member_attack_usage';
  ...
  // Advice / "why" questions are not lookups: "who should we promote to elder"
  // must not be answered with the current elder list as if it were the answer.
  const isAdvice = /\b(?:should|shall|why|suggest|recommend|advice|improve|strategy|strategies|tips?|best way|how (?:can|do|to)|worth)\b/.test(q);
  if (isAdvice && operation === 'role_members') operation = 'general';
  ...
  return { scope, operation, intent, is_advice: isAdvice, identity_field: identityField, ...filters, normalized: q };
}
...
export function applyAttackUsageFilters(rows, plan) {
  ...
  if (plan.attacks_remaining_any) {
    filtered = filtered.filter(r => Number(r.attacks_available ?? 0) - Number(r.attacks_used ?? 0) > 0);
  }
...
export function executeIntent(question, { players = [], currentWarMembers = [] } = {}) {
  ...
  if (plan.operation === 'member_attack_usage') {
    ...
    return result ? {
      ...
      scope: plan.scope,
      query: 'attack_usage',
      filter: { unused: plan.unused, attacks_remaining_any: plan.attacks_remaining_any, attacks_used: plan.attacks_used, attacks_used_min: plan.attacks_used_min ?? null, attacks_remaining: plan.attacks_remaining },
