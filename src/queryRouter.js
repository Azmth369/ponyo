...
function attackUsageResult(plan, rows) {
  ...
  return {
    ...
    filter: {
      unused: plan.unused,
      attacks_remaining_any: plan.attacks_remaining_any ?? false,
...
export async function runDeterministicQuery(question) {
  ...
  if (plan.operation === 'member_metric') {
    ...
    else if (!plan.asks_count && plan.sort) rows = rows.slice(0, 1);
  ...
  if (plan.scope === 'capital') {
    ...
    if (plan.operation === 'member_attack_usage') {
      ...
      if (plan.attacks_remaining_any) {
        // Attackers with attacks left + roster members who never attacked.
        const partial = applyAttackUsageFilters(members, plan).map(r => ({
          name: r.player_name, tag: r.player_id,
          attacks_used: Number(r.attacks_used ?? 0), attacks_available: Number(r.attacks_available ?? 0),
          attacks_remaining: Math.max(Number(r.attacks_available ?? 0) - Number(r.attacks_used ?? 0), 0)
        }));
        const never = (season.absentees ?? []).map(name => ({ name, attacks_used: 0, attacks_available: 0, attacks_remaining: 0 }));
        return {
          intent: 'structured_clan_query', scope: 'capital', query: 'attack_usage',
          filter: { unused: false, attacks_remaining_any: true, attacks_used: null, attacks_used_min: null, attacks_remaining: null },
          result_count: partial.length + never.length,
          result: [...partial, ...never],
          event,
          note: 'Includes members who attacked but still have attacks left, plus roster members who have not attacked at all (attack counts unknown for those).'
        };
      }
      return { ...attackUsageResult(plan, members), event };
    }
